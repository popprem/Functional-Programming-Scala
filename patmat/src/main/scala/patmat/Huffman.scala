...
object Huffman {
  ...
  def decodeHelper(root: CodeTree, curtree: CodeTree, bits: List[Bit]): List[Char] = bits match {
    case Nil => curtree match {
      case Fork(_, _, _, _) => Nil
      case Leaf(char, _) => List(char)
    }
    case _ => curtree match {
      case Fork(left, right, _, _) => bits.head match {
        case 0 => decodeHelper(root, left, bits.tail)
        case 1 => decodeHelper(root, right, bits.tail)
      }
      case Leaf(char, _) => List(char) ::: decodeHelper(root, root, bits)
    }
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = text match {
    case Nil => Nil
    case elem :: rest => encodeChar(tree, elem) ::: encode(tree)(rest)
  }
  def encodeChar(tree: CodeTree, char: Char): List[Bit] = tree match {
    case Leaf(c, _) => Nil
    case Fork(left, right, _, _) => {
      if (containChar(left, char)) 0 :: encodeChar(left, char)
      else if (containChar(right, char)) 1 :: encodeChar(right, char)
      else Nil
    }
  }
